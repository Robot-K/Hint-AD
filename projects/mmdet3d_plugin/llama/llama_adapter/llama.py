...
class RMSNorm(torch.nn.Module):
    ...
    def _norm(self, x):
        x = torch.clamp(x, min=-1e8, max=1e8) # modified for stability
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], prompt=None):
        if torch.isinf(x).any():
            print(f"Inf detected before attention of layer {self.layer_id}") # modified
        x_norm = self.attention_norm(x)
        if torch.isinf(x_norm).any():
            print(f"Inf detected at x_norm of layer {self.layer_id}") # modified
        h = x + self.attention.forward(x_norm, start_pos, freqs_cis, mask, prompt)
        if torch.isinf(h).any():
            print(f"Inf detected at attention of layer {self.layer_id}") # modified
        out = h + self.feed_forward.forward(self.ffn_norm(h))
        if torch.isinf(out).any():
            print(f"Inf detected at feed_forward of layer {self.layer_id}") # modified
        return out
