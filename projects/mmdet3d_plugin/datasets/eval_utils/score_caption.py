...
def extract_object_counts(input_string):
    CLASS = ['car', 'truck', 'construction_vehicle', 'bus', 'bicycle', 'pedestrian']
    CLASSES = ['cars', 'trucks', 'construction_vehicles', 'buses', 'bicycles', 'pedestrians']
    count_list = np.zeros((len(CLASSES),), dtype=int)
    pattern = r"(\d+)\s+(\w+)"
    matches = re.findall(pattern, input_string)
    for number, object_class in matches:
        positions = find_positions(CLASS, object_class)
        if len(positions) > 0:
...
def score_align_count(cap_list, json_file):
    ...
    for caption in cap_list:
        ...
        count_car_pred.append(count_list_pred[-1])
        count_car_gt.append(count_list_gt[-1])
        root_mean_square = np.sqrt(np.sum((count_list_pred - count_list_gt)**2) / len(count_list_pred))
        complexity_list.append(np.sum(count_list_gt).item())
    ...
    # 绘制散点图
    ax.scatter(count_car_gt, count_car_pred, s=50, alpha=0.6)

    # 添加标题和轴标签
    ax.set_title('Scatter Plot of Car Counts')
    ax.set_ylabel('Predicted Car Counts')
    ax.set_xlabel('Ground Truth Car Counts')

    # 调整坐标轴范围以适应数据
    ax.set_ylim(min(count_car_pred) - 1, max(count_car_pred) + 1)
    ax.set_xlim(min(count_car_gt) - 1, max(count_car_gt) + 1)

    # 保存图像到文件
    plt.savefig('temp/scatter.png', dpi=300, bbox_inches='tight')
...
def score_align_plan(cap_list, json_file):
    ...
    for caption in cap_list:
        ...
        distance = distances[-1] if len(distances) > 0 else 0
        trajectory_diff_list.append(distance)
    
        complexity_list.append(scene_complexity)
    
    mean_distance = np.mean(trajectory_diff_list) if len(trajectory_diff_list) > 0 else 0
    with open(json_file.replace('.json', '_align_plan.json'), 'w') as f:
        align_plan_dict = dict(complexity_list=complexity_list, trajectory_diff_list=trajectory_diff_list,
            mean_distances=mean_distance)
        json.dump(align_plan_dict, f)
    print(f"mean_trajectory_disalignment = {mean_distance}")
...
def score_caption(json_file):
        ...
        for batch_index in range(batch_size):
            if '<caption>' in question[batch_index]:
                try:
                    ...
                    caption_list.append([pred_narration+pred_reasoning, gt_narraion+gt_reasoning])
                except:
                    pass
    ...
    # with open('temp/caption.pkl', 'wb') as file:
    #     pkl.dump(caption_list, file)
    
    # print("=========== Scoring Narration ===========")
    # score(narration_list)
    # print("=========== Scoring Reasoning ===========")
    # score(reasoning_list)
    # print("=========== Scoring Captioning ===========")
    # score(caption_list)
    # print("=========== Scoring tod3 ===========")
    # score(tod3_list)
