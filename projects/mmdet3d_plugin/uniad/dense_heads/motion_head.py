...
class MotionHead(BaseMotionHead):
    ...
    def forward_train(self,
        ...
        def filter_vehicle_query(outs_motion, all_matched_idxes, gt_labels_3d, vehicle_id_list):
            ...
            outs_motion['all_traj_preds'] = outs_motion['all_traj_preds'][:, :, vehicle_mask>0] # Kding
            all_matched_idxes[0] = all_matched_idxes[0][vehicle_mask>0]
            return outs_motion, all_matched_idxes, vehicle_mask, query_label

        all_matched_idxes[0] = all_matched_idxes[0][:-1]
        outs_motion['sdc_traj_query'] = outs_motion['traj_query'][:, :, -1]         # [3, 1, 6, 256]     [n_dec, b, n_mode, d]
        outs_motion['sdc_traj_preds'] = outs_motion['all_traj_preds'][:, :, -1]
        ...
        outs_motion['all_traj_preds'] = outs_motion['all_traj_preds'][:, :, :-1] # Kding
        
        outs_motion, all_matched_idxes, vehicle_mask, query_label = filter_vehicle_query(outs_motion, all_matched_idxes, gt_labels_3d, self.vehicle_id_list)
        outs_motion['all_matched_idxes'] = all_matched_idxes
        outs_motion['query_label'] = query_label # modi: to get label for further analysis
        outs_motion['vehicle_mask'] = vehicle_mask # modi: to get vehicle mask for further analysis
    ...
    def forward_test(self, bev_embed, outs_track={}, outs_seg={}):
        ...
        def filter_vehicle_query(outs_motion, labels, vehicle_id_list):
            ...
            outs_motion['all_traj_preds'] = outs_motion['all_traj_preds'][:, :, vehicle_mask>0] # Kding
            return outs_motion, vehicle_mask, labels
        
        outs_motion, vehicle_mask, labels = filter_vehicle_query(outs_motion, labels, self.vehicle_id_list)
        ...
        outs_motion['all_traj_preds'] = outs_motion['all_traj_preds'][:, :, :-1] # Kding
        outs_motion['query_label'] = labels[:-1] # modi: to get label for further analysis
        outs_motion['vehicle_mask'] = vehicle_mask[:-1] # modi: to get vehicle mask for further analysis
    ...
    def forward(self, 
        ...
        - 'all_traj_preds': A tensor of shape (num_levels, B, A_track, num_points, num_future_steps, 5) with predicted trajectories for each level.
