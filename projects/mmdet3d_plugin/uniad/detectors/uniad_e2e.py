...
class UniAD(UniADTrack):
    ...
    def __init__(
        ...
        freeze_module_list = ['img_backbone'],
        freeze_bn=False,
        **kwargs,
    ):
        ...
        if caption_head:
            self.caption_head = build_head(caption_head)
        ...
        assert set(task_loss_weight.keys()) == \
               {'track', 'occ', 'motion', 'map', 'planning', 'caption'} # modified
    ...
    def with_caption_head(self):
        return hasattr(self, 'caption_head') and self.caption_head is not None # modified
    ...
    def forward_train(self,
                      img=None,
                      img_metas=None,
                      gt_bboxes_3d=None, # 9d x, y, z, sx, sy, sz, yaw, vx, vy
                      gt_labels_3d=None,
                      gt_inds=None,
                      l2g_t=None,
                      l2g_r_mat=None,
        ...
        """
        losses = dict() # modi: no skip mechanism for modi
        ...
        if self.with_occ_head:
            if outs_motion['track_query'].shape[1] == 0:
                ...
                outs_motion['all_traj_preds'] = torch.zeros((3, 1, 1, 6, 12, 5)).to(bev_embed) # modified
        ...
        if self.with_caption_head:
            losses_caption = self.caption_head.forward_train(bev_embed, gt_caption_token, gt_caplabel, ins_list, tod3_centers, outs_track, outs_motion, outs_planning['outs_motion'])
    ...
    def forward_test(self,
        ...
        if self.with_caption_head:
            result_caption = self.caption_head.forward_test(bev_embed, result_track[0], outs_motion, outs_planning=result_planning, gt_caption_token=gt_caption_token, gt_caplabel=gt_caplabel, ins_list=ins_list, tod3_centers=tod3_centers)
            result[0]['caption_results'] = result_caption
        ...
        # update for each frame in queue_length, which is always 1 here. The prev_bev and test_track_instances are updated in an RNN way.
