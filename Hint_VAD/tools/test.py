...
def parse_args():
    ...
    parser.add_argument('--results', help='load output results')
    parser.add_argument(
        '--fuse-conv-bn',
        action='store_true',
...
def main():
    ...
    if rank == 0:
        if args.out: # cby
            ...
            #     mmcv.dump(outputs, args.out)
            # else:
            #     mmcv.dump(outputs['bbox_results'], args.out) cby
        ...
        if args.eval:
            ...
            print('Results length:', len(outputs['bbox_results']))
            # NOTE: record to json cby
            json_path = args.json_dir
            if not os.path.exists(json_path):
                os.makedirs(json_path)
            eval_kwargs['out_dir'] = json_path
            eval_kwargs['show'] = args.show
