...
from projects.mmdet3d_plugin.datasets.eval_utils.score_caption import score_caption # cby
from shapely import affinity, ops
from shapely.errors import TopologicalError
from shapely.geometry import LineString, box, MultiPolygon, MultiLineString, Polygon, GeometryCollection
...
class VADCustomNuScenesDataset(NuScenesDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.QA = QA
        self.command = command # cby
        self.DriveLM = DriveLM
        self.tod3 = tod3
    ...
    def get_data_info(self, index):
        ...
        if self.caption and 'caption' in info:
            input_dict['caption'] = info['caption']
        if self.QA and 'QA' in info:
            input_dict['QA'] = info['QA']
        if self.command and 'customized_command' in info:
            input_dict['customized_command'] = info['customized_command'] #cby
        if self.DriveLM and 'DriveLM' in info:
            input_dict['DriveLM'] = info['DriveLM']
        if self.tod3 and 'tod3_tokens' in info:
            input_dict['tod3_tokens'] = info['tod3_tokens']
        input_dict['tod3_centers'] = info['gt_boxes']
    ...
    def evaluate(self,
        ...
        Args:
            results ([dict]): Testing results of the dataset. keys: 'bbox_results',
                'caption_results' (optional)
        ...
        if self.caption:
            mmcv.mkdir_or_exist(osp.join(out_dir, 'caption'))
            with open(osp.join(out_dir, 'caption', 'caption_results.json'), 'w') as f:
                json.dump(results['caption_results'], f)
            score_caption(osp.join(out_dir, 'caption', 'caption_results.json'))
        ...
        for i in range(len(results['bbox_results'])):
            for key in all_metric_dict.keys():
                all_metric_dict[key] += results['bbox_results'][i]['metric_results'][key]
        ...
        metric_dict = None
        num_valid = 0
        for res in results['bbox_results']:
        ...
        if show:
            self.show(results['bbox_results'], out_dir, pipeline=pipeline)
        return results_dict
