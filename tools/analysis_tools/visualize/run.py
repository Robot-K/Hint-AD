...
class Visualizer:
    ...
    def __init__(
        ...
        self.show_caption = show_caption # cby
    ...
    def _parse_predictions_multitask_pkl(self, predroot):
        ...
        if self.show_caption: #cby
            caption_all = outputs['caption_results']
    ...
    def combine(self, sample_token, out_filename):
        ...
        if self.show_caption: #cby
            merge_image = self.add_caption(sample_token, merge_image)
        cv2.imwrite(out_filename + '.jpg', merge_image)
        os.remove(out_filename + '_cam.jpg')
        
    def add_caption_old(self, sample_token, image): # cby
        caption = self.predictions[sample_token]['caption']
        text1 = caption[1][0]
        textQA = []
        textCaption = []
        for question, answer, gt_answer in zip(caption[0], caption[1], caption[2]):
            if '<align' in question or '<DriveLM' in question or 'QA' in question:
                pattern = r'question: (.*?)(?=\n###)'
                match = re.search(pattern, question, re.IGNORECASE | re.DOTALL)
                if match:
                    question = match.group(1).strip()   
                else:
                    question = 'Please predict the control signal of the ego car.'
                textQA.append([question, answer, gt_answer])
            else:
                textCaption.append([answer, gt_answer])
        ...
        text_size = cv2.getTextSize(text1, font, font_scale, thickness1)[0]
        text_width, text_height = text_size
        image_height, image_width = image.shape[:2]
        new_height = image_height + text_height * (7 * len(textQA) + 5 * len(textCaption) + 1)
        new_image = 255 * np.ones((new_height, image_width, 3), dtype=np.uint8)
        new_image[:image_height, :, :] = image

        for i, pair in enumerate(textQA):
            text_origin = (200, image_height + text_height * (2+7*i))
            cv2.putText(new_image, f'Question: ', text_origin, font, font_scale, color3, thickness2)
            text_origin = (800, image_height + text_height * (2+7*i))
            cv2.putText(new_image, pair[0], text_origin, font, font_scale, color3, thickness1)
            text_origin = (200, image_height + text_height * (4+7*i))
            cv2.putText(new_image, f'Answer: ', text_origin, font, font_scale, color3, thickness2)
            text_origin = (800, image_height + text_height * (4+7*i))
            cv2.putText(new_image, pair[1], text_origin, font, font_scale, color3, thickness1)
            text_origin = (200, image_height + text_height * (6+7*i))
            cv2.putText(new_image, f'GT: ', text_origin, font, font_scale, color3, thickness2)
            text_origin = (800, image_height + text_height * (6+7*i))
            cv2.putText(new_image, pair[2], text_origin, font, font_scale, color3, thickness1)

        for i, pair in enumerate(textCaption):
            text_origin = (200, image_height + text_height * 7 * len(textQA) + text_height * (2+5*i))
            cv2.putText(new_image, f'Answer: ', text_origin, font, font_scale, color3, thickness2)
            text_origin = (800, image_height + text_height * 7 * len(textQA) + text_height * (2+5*i))
            cv2.putText(new_image, pair[0], text_origin, font, font_scale, color3, thickness1)
            text_origin = (200, image_height + text_height * 7 * len(textQA) + text_height * (4+5*i))
            cv2.putText(new_image, f'GT: ', text_origin, font, font_scale, color3, thickness2)
            text_origin = (800, image_height + text_height * 7 * len(textQA) + text_height * (4+5*i))
            cv2.putText(new_image, pair[1], text_origin, font, font_scale, color3, thickness1)
        return new_image

    def to_video_old(folder_path, out_path, fps=4, downsample=1, caption_frame=None): #cby
        ...
        for img_path in imgs_path:
            ...
            print(img_id, img_id_pre, img_id_start)
            if img_id - img_id_pre != 1:
                img_id_start = img_id
                caption_img = img[height-caption_height:height, :, :]
            elif (img_id - img_id_start)%caption_frame == 0:
                caption_img = img[height-caption_height:height, :, :]
            else:
    ...
    def add_caption(self, sample_token, image): # cby
        caption = self.predictions[sample_token]['caption']
        text1 = caption[1][0]
        textQA = []
        textCaption = []
        for question, answer, gt_answer in zip(caption[0], caption[1], caption[2]):
            if '<align' in question or '<DriveLM' in question or 'QA' in question:
                ...
                if '<caption>' in question:
                    textCaption.append(['Narration & Reasoning', answer, gt_answer])
                if '<command>' in question:
                    textCaption.append(['Command Prediction', answer, gt_answer])
        ...
        text_width, text_height = text_size
        image_height, image_width = image.shape[:2]
        new_height = image_height + text_height * (7 * 5)
        new_image = 255 * np.ones((new_height, image_width, 3), dtype=np.uint8)
        new_image[:image_height, :, :] = image

        for i, pair in enumerate(textCaption):
            text_origin = (200, image_height + text_height * (2+7*i))
            cv2.putText(new_image, pair[0], text_origin, font, font_scale, color3, thickness2)
            text_origin = (200, image_height + text_height * (4+7*i))
            cv2.putText(new_image, f'Answer: ', text_origin, font, font_scale, color3, thickness2)
            text_origin = (800, image_height + text_height * (4+7*i))
            cv2.putText(new_image, pair[1], text_origin, font, font_scale, color3, thickness1)
        ...
        for i in range(len(textCaption), min(5, len(textQA)+len(textCaption))):
            pair = textQA[i-len(textCaption)]
            text_origin = (200, image_height + text_height * (2+7*i))
            cv2.putText(new_image, f'Question: ', text_origin, font, font_scale, color3, thickness2)
            text_origin = (800, image_height + text_height * (2+7*i))
            cv2.putText(new_image, pair[0], text_origin, font, font_scale, color3, thickness1)
...
def main(args):
    ...
    for i in range(len(viser.nusc.sample)):
        ...
        if args.project_to_cam:
            ...
            viser.combine(sample_token, os.path.join(args.out_folder, str(i).zfill(3)))

    viser.to_video(args.out_folder, args.demo_video, fps=4, downsample=2, caption_frame=3)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--predroot', help='Path to results.pkl')
    parser.add_argument('--out_folder', help='Output folder path')
    parser.add_argument('--demo_video', help='Demo video path, containing *')
