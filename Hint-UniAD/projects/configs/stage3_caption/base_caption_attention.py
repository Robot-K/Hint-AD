...
llama_ckpt_root = "ckpts/llama-2-7b"
llama_tokenzier_root = plugin_dir + "llama/tokenizer.model" # modified
caption_anno_path = f'data/nuscenes/tod3/final_caption_bbox_token.json' # modi
...
model = dict(
    ...
    use_grid_mask=True,
    image_mask_ratio = [0,0.,0.,0.,0.,0.], # front, front_right, front_left, behind, behind_left, behind_right modi: not needed now
    ...
    freeze_module_list = ['img_backbone', 'img_neck', 'memory_bank', 'query_embedding', 'query_interact', 'reference_points', 'pts_bbox_head', 'seg_head', 'motion_head', 'occ_head', 'planning_head'], # modified
    freeze_bn = True,
    score_thresh=0.4,
    ...
    caption_head=dict(
        type='CaptionHead',
        llama_ckpt_root=llama_ckpt_root,
        llama_tokenzier_root=llama_tokenzier_root,
        train_cfg = dict(ins_masked=0., sdc_masked=0., planning_masked=0.),
        test_cfg = dict(ins_masked=False, sdc_masked=False, planning_masked=False),
        tod3_ratio=0.35*(1 if with_tod3 else 0),
        caption_ratio=0.35,
        iou_threshold=0.25,
        max_qa_num = max_qa_num, # modi
        tokenizer_path = llama_tokenzier_root,
        caption_anno_path = caption_anno_path,
    ), # modified
...
ann_file_train=info_root + f"nuscenes_infos_capQAcmdDriveLMtod3_train.pkl"
ann_file_val=info_root + f"nuscenes_infos_capQAcmdDriveLMtod3_val.pkl"
ann_file_test=info_root + f"nuscenes_infos_capQAcmdDriveLMtod3_val.pkl"
...
train_pipeline = [
    ...
    dict(type="LoadCaption", tokenizer_path=llama_tokenzier_root, caption_anno_path=caption_anno_path), # modified, modi
    dict(
...
data = dict(
    ...
    train=dict(
        ...
        QA=with_QA,
        command=with_command, # modified
        DriveLM=with_DriveLM,
        tod3=with_tod3,
    ...
    val=dict(
        ...
        classes=class_names,
        modality=input_modality,
        eval_mod=['det', 'map', 'track', 'motion', 'caption'], # modified
        occ_receptive_field=3,
        occ_n_future=occ_n_future_max,
        occ_filter_invalid_sample=False,
