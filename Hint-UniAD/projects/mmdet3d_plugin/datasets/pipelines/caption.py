...
class LoadCaption(object):
    ...
    def __call__(self, input_dict): # modi
        ...
        if 'command' in input_dict:
            instruction = "<command> Information of the car's bird's eye view video frames, tracks, and trajectories are also (optionally) provided, please predict the control signal of the ego car. Choose from: 'TURN RIGHT', 'TURN LEFT', and 'FORWARD'"
            instruction = utils.format_prompt(instruction, None)
            if not (input_dict['command'] == 2 and np.random.rand() < 0.5): # for category balance
                input1_list.append(instruction)
                if input_dict['command'] == 0:
                    command = 'TURN RIGHT'
                if input_dict['command'] == 1:
                    command = 'TURN LEFT'
                else:
                    command = 'FORWARD'
                answer_list.append(command)
                ins_list.append(-3)
