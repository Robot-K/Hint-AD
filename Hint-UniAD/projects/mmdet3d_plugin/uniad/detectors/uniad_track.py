...
from projects.mmdet3d_plugin.models.utils.grid_mask import GridMask, WholeImageMask
import torchvision.transforms as transforms # modified
import copy
import math
...
class UniADTrack(MVXTwoStageDetector):
    ...
    def __init__(
        self, 
        use_grid_mask=False,
        image_mask_ratio = [0,0.3,0.3,0.3,0.3,0.3], # front, front_right, front_left, behind, behind_left, behind_right
        img_backbone=None,
        img_neck=None,
        pts_bbox_head=None,
        train_cfg=None,
        ...
        self.grid_mask = GridMask(
            True, True, rotate=1, offset=False, ratio=0.3, mode=1, prob=0.5
        )
        self.whole_image_mask = WholeImageMask(image_mask_ratio)
    ...
    def extract_img_feat(self, img, len_queue=None):
        ...
        img = self.whole_image_mask(img)
        # self.save_images_from_tensor(img, f'./temp/pictures') # modified, for debug
    ...
    def save_images_from_tensor(self, image_tensor, output_dir): # this function is for debug
        ...
        scaled_image_tensor = (image_tensor - min_val) / (max_val - min_val)
        for i in range(n):
            image = scaled_image_tensor[i]
            pil_image = to_pil(image)
            output_path = os.path.join(output_dir, f"{i}.jpg")
            pil_image.save(output_path)

        print(f"image {n} is saved to: {output_dir}")
    ...
    def _forward_single_frame_train(
        ...
        velo = output_coords[-1, 0, :, -2:]  # [num_query, 2] # modified
        ...
        for i in range(nb_dec):
            ...
            # all_instances_pred_boxes.append(output_coords[i, 0])   # Not used # modified
