...
def custom_multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False):
        ...
        with torch.no_grad():
            ...
            if isinstance(result, dict):
                ...
                if eval_caption and 'caption_results' in result.keys():
                    ...
                    caption_results.extend(caption_result)
            else:
    ...
    print('finish reading results')

    # collect results from all ranks
    if gpu_collect:
        print('collecting results from gpu')
        bbox_results = collect_results_gpu(bbox_results, len(dataset))
        if have_mask:
            mask_results = collect_results_gpu(mask_results, len(dataset), prefix='mask')
        else:
            mask_results = None
        if eval_caption:
            caption_results = collect_results_gpu(caption_results, len(dataset), prefix='caption')
    else:
        print('collecting results from cpu')
        bbox_results = collect_results_cpu(bbox_results, len(dataset), tmpdir)
        tmpdir = tmpdir+'_mask' if tmpdir is not None else None
        if have_mask:
            mask_results = collect_results_cpu(mask_results, len(dataset), tmpdir, prefix='mask')
        else:
            mask_results = None
        if eval_caption:
            caption_results = collect_results_cpu(caption_results, len(dataset), tmpdir, prefix='caption')

    print('finish collecting results')
    if eval_planning:
    ...
    print('finish computing results')
    return ret_results
